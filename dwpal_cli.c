...
int main(int argc, char *argv[])
{
	...
	*/
	...
	{  /* one-shot mode */
		...
		while ( ((option = getopt(argc, argv, "i:m:v:c:l:d:p:s:")) != -1) && (optarg != NULL) )
		{
			...
			switch (option)
			{
				...
				case 'm':
					stringLength = strnlen_s(optarg, sizeof(mainInterface));
					...
					if ( (!strncmp(mainInterface, "Main", 7)) || (!strncmp(mainInterface, "MAIN", 7)) || (!strncmp(mainInterface, "main", 7)) )
					{
				...
				case 'v':
					...
					stringLength = strnlen_s(optarg, sizeof(oneShotVAPName[numOfListeningVaps]));
				...
				case 's':
					stringLength = strnlen_s(optarg, DWPAL_FIELD_NAME_LENGTH);
